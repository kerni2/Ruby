...
class Rail
  ...
  def create_station
    ...
    if Station.new(station_name).validate?
  ...
  def create_passenger_train
    ...
    if PassengerTrain.new(train_number).validate?
  ...
  def create_cargo_train
    puts 'Введите номер поезда:'
    train_number = gets.chomp.to_s
    if CargoTrain.new(train_number).validate?
  ...
  def train_add_wagon
    ...
    if need_train(train_number).nil?
      ...
      if new_wagon.validate? || wagon_seats.to_i.positive?
      ...
      if new_wagon.validate? || wagon_volume.to_i.positive?
        need_train(train_number).wagons << new_wagon
  ...
  def create_route
    ...
    if Route.new(start, stop, number).validate?
